...
static void normalize_parser_intent(char *line) {
  ...
  if (!strcmp(line, "my gear") || !strcmp(line, "show gear")) {
    rewrite_command(line, "gear", "");
    return;
  }
  if (!strcmp(line, "show equipment") || !strcmp(line, "my equipment")) {
    rewrite_command(line, "equipment", "");
    return;
  }
  if (!strcmp(line, "my loadout") || !strcmp(line, "show loadout") ||
...
static void eq_sync_pc_sheet(void) {
  ...
  if (g_eq_slots[EQ_WEAPON][0])
    snprintf(p.weapon, sizeof p.weapon, "%s", g_eq_slots[EQ_WEAPON]);
  else
    snprintf(p.weapon, sizeof p.weapon, "None");
  if (g_eq_slots[EQ_CHEST][0])
    snprintf(p.armor, sizeof p.armor, "%s", g_eq_slots[EQ_CHEST]);
  else
    snprintf(p.armor, sizeof p.armor, "None");
  pc_restore(&p);
}
...
static void run_equipment_inventory_ui(const char *pending_acc, int *did_fullscreen) {
  ...
  for (;;) {
    ...
    for (i = 0; i < EQ_SLOT_COUNT; i++) {
      ...
      cat = aet_item_catalog_by_slug(g_eq_slots[i]);
      if (cat) {
        def += cat->def;
        atk += cat->atk;
        wgt += cat->wgt;
    ...
    for (i = 0; i < EQ_SLOT_COUNT; i++) {
      ...
      if (strlen(tag) < 10) {
        ...
        ui_line_append(&lb, " ");
      if (g_eq_slots[i][0]) {
        const AetItemCatalogEntry *cat = aet_item_catalog_by_slug(g_eq_slots[i]);
        CraftMatProfile sp;
        char stat[24];
        if (cat) {
          sdef = cat->def;
        ...
        ui_line_append_width(
            &lb,
            (cat && cat->label && cat->label[0]) ? cat->label : g_eq_slots[i], 20);
    ...
    for (i = 0; i < EQ_INV_ROWS && i < g_inv_n; i++) {
      ...
      disp_name = (cat && cat->label && cat->label[0]) ? cat->label : g_inv[i];
