...
static void strip_natural_prefixes(char *line) {
  ...
  while (guard++ < 10) {
    ...
    if (!strncmp(line, "i would like to ", 16)) {
      memmove(line, line + 16, strlen(line + 16) + 1);
      continue;
    }
    if (!strncmp(line, "i'd like to ", 12)) {
      memmove(line, line + 12, strlen(line + 12) + 1);
      continue;
    }
    if (!strncmp(line, "i'm trying to ", 14)) {
      memmove(line, line + 14, strlen(line + 14) + 1);
      continue;
    }
    if (!strncmp(line, "im trying to ", 13)) {
      memmove(line, line + 13, strlen(line + 13) + 1);
      continue;
    }
    if (!strncmp(line, "i want to ", 10)) {
      memmove(line, line + 10, strlen(line + 10) + 1);
      continue;
    }
    if (!strncmp(line, "could you please ", 17)) {
      memmove(line, line + 17, strlen(line + 17) + 1);
      continue;
    }
    if (!strncmp(line, "would you please ", 17)) {
      memmove(line, line + 17, strlen(line + 17) + 1);
      continue;
    }
    if (!strncmp(line, "could you ", 10)) {
      memmove(line, line + 10, strlen(line + 10) + 1);
      continue;
    }
    if (!strncmp(line, "would you ", 10)) {
      memmove(line, line + 10, strlen(line + 10) + 1);
      continue;
    }
    if (!strncmp(line, "could i please ", 15)) {
      memmove(line, line + 15, strlen(line + 15) + 1);
      continue;
    }
    if (!strncmp(line, "could i ", 8)) {
      memmove(line, line + 8, strlen(line + 8) + 1);
      continue;
    }
    if (!strncmp(line, "can i please ", 13)) {
      memmove(line, line + 13, strlen(line + 13) + 1);
      continue;
    }
    if (!strncmp(line, "can i ", 6)) {
      memmove(line, line + 6, strlen(line + 6) + 1);
      continue;
    }
    if (!strncmp(line, "let me please ", 14)) {
      memmove(line, line + 14, strlen(line + 14) + 1);
      continue;
    }
    if (!strncmp(line, "let me ", 7)) {
      memmove(line, line + 7, strlen(line + 7) + 1);
      continue;
    }
    if (!strncmp(line, "please ", 7)) {
      memmove(line, line + 7, strlen(line + 7) + 1);
      continue;
    }
    if (!strncmp(line, "i need to ", 10)) {
      memmove(line, line + 10, strlen(line + 10) + 1);
      continue;
    }
    if (!strncmp(line, "i should ", 9)) {
      memmove(line, line + 9, strlen(line + 9) + 1);
      continue;
    }
    if (!strncmp(line, "try to ", 7)) {
      memmove(line, line + 7, strlen(line + 7) + 1);
      continue;
    }
    if (!strncmp(line, "try and ", 8)) {
      memmove(line, line + 8, strlen(line + 8) + 1);
      continue;
    }
    break;
...
static void age_disclaimer_wait(void) {
  ...
  ui_block_pause("18+ ADULT CONTENT", text);
}
...
static void fill_help_text(char *buf, size_t cap) {
  snprintf(
      ...
      "          loadout | gear | equipment | outfit | traits | trait | personality\n"
...
static int drop_item(const char *name, char *msg, size_t msgcap) {
  ...
  for (j = 0; j < EQ_SLOT_COUNT; j++) {
    if (str_ieq(g_eq_slots[j], dropped)) g_eq_slots[j][0] = '\0';
  }
...
static void format_loadout_body(char *body, size_t cap) {
  ...
  snprintf(
      ...
      "Loadout\n\n"
      "%s\n\n"
      "Fancier editions track nested equipment slots, weight, and swap sets.\n",
      banner);
  body_append(
      body, cap,
      "Here you get a clean operational picture: what you declared at creation,\n"
      "what you have in hand, and how full your pack is — enough to role-play\n"
      "without maintaining a spreadsheet.\n\n");
  body_append(body, cap, "Stated kit (from character sheet)\n"
                          "  Armor:  %s\n"
                          "  Weapon: %s\n\n",
              equip_display(p.armor), equip_display(p.weapon));
  if (g_ready_item[0] && inv_has(g_ready_item))
    rdy = g_ready_item;
  else
    rdy = NULL;
  body_append(body, cap, "In play right now\n"
                          "  Readied / in hand: %s\n\n",
              rdy ? rdy : "— (equip | hold | wield <item>)");
  body_append(body, cap,
              "Pack pressure\n"
              "  Carrying %d / %d inventory slots  (inventory  for the full list)\n"
              "  Tip: lights, tools, and trade goods all compete for the same bag.\n\n"
              "Mods can append encumbrance fiction, set bonuses, or episode gear "
              "below.\n",
              g_inv_n, MAX_INV);
}
...
static void normalize_parser_intent(char *line) {
  ...
  if (!strncmp(line, "remember that ", 14)) {
    rewrite_command(line, "note", line + 14);
    return;
  }
  if (!strncmp(line, "jot down ", 9)) {
    rewrite_command(line, "note", line + 9);
    return;
  }
  if (!strncmp(line, "jot ", 4)) {
    rewrite_command(line, "note", line + 4);
    return;
  }
  if (!strncmp(line, "memo ", 5)) {
    rewrite_command(line, "note", line + 5);
    return;
  }
  if (!strcmp(line, "im stuck") || !strcmp(line, "i am stuck") ||
      !strcmp(line, "i'm stuck")) {
    rewrite_command(line, "hints", "");
    return;
  }
  if (!strcmp(line, "give me a hint") || !strcmp(line, "give me hint") ||
      !strcmp(line, "nudge") || !strcmp(line, "i need a hint") ||
      !strcmp(line, "another hint")) {
    rewrite_command(line, "hints", "");
    return;
  }
  if (!strcmp(line, "what changed last turn") ||
      !strcmp(line, "what changed") ||
      !strcmp(line, "what happened last turn")) {
    rewrite_command(line, "causality lastturn", "");
    return;
  }
  if (!strcmp(line, "why did that happen") ||
      !strcmp(line, "why did this happen") ||
      !strcmp(line, "why did that occur")) {
    rewrite_command(line, "causality recent", "");
    return;
  }
  if (!strcmp(line, "why did npc react") || !strcmp(line, "why npc reacted") ||
      !strcmp(line, "why did they react")) {
    rewrite_command(line, "causality social", "");
    return;
  }
  if (!strcmp(line, "what triggered that") || !strcmp(line, "what caused that") ||
      !strcmp(line, "explain what happened")) {
    rewrite_command(line, "causality explain", "");
    return;
  }
  if (!strcmp(line, "who heard me") || !strcmp(line, "did anyone hear me")) {
    rewrite_command(line, "causality social", "");
    return;
  }
  if (!strcmp(line, "who noticed me") || !strcmp(line, "did anyone notice me") ||
      !strcmp(line, "what heard that noise")) {
    rewrite_command(line, "noise", "");
    return;
  }
  if (!strcmp(line, "why didnt save work") || !strcmp(line, "why didn't save work") ||
      !strcmp(line, "why load failed")) {
    rewrite_command(line, "causality save", "");
    return;
  }
  if (!strcmp(line, "why parser failed") || !strcmp(line, "why command failed to parse")) {
    rewrite_command(line, "causality parser", "");
    return;
  }
  if (!strcmp(line, "why did movement fail") || !strcmp(line, "why cant i go") ||
      !strcmp(line, "why can't i go")) {
    rewrite_command(line, "why blocked", "");
    return;
  }

  if (!strcmp(line, "what can i do") || !strcmp(line, "what should i do") ||
      !strcmp(line, "what can you do") || !strcmp(line, "commands")) {
    rewrite_command(line, "help", "");
    return;
  }
  if (!strcmp(line, "crafting") || !strcmp(line, "open forge") ||
      !strcmp(line, "material forge") || !strcmp(line, "forge menu")) {
    rewrite_command(line, "forge", "");
    return;
  }
  if (!strcmp(line, "instructions") || !strcmp(line, "how do i play") ||
      !strcmp(line, "show help") || !strcmp(line, "keyboard shortcuts") ||
      !strcmp(line, "game controls")) {
    rewrite_command(line, "help", "");
    return;
  }
  if (!strcmp(line, "modding guide") || !strcmp(line, "mod guide") ||
      !strcmp(line, "mods guide") || !strcmp(line, "dlc guide") ||
      !strcmp(line, "dlc help")) {
    rewrite_command(line, "help modding", "");
    return;
  }
  if (!strcmp(line, "who made this") || !strcmp(line, "developers") ||
      !strcmp(line, "attribution")) {
    rewrite_command(line, "about", "");
    return;
  }
  if (!strcmp(line, "identity")) {
    rewrite_command(line, "character brief", "");
    return;
  }
  if (!strcmp(line, "who am i") || !strcmp(line, "whoami") ||
      !strcmp(line, "what do i look like") || !strcmp(line, "describe me") ||
      !strcmp(line, "describe myself") || !strcmp(line, "my appearance") ||
      !strcmp(line, "my character") || !strcmp(line, "show character") ||
      !strcmp(line, "show me my character") || !strcmp(line, "look at me") ||
      !strcmp(line, "look at myself")) {
    rewrite_command(line, "character", "");
    return;
  }
  if (!strcmp(line, "my skills") || !strcmp(line, "show skills") ||
      !strcmp(line, "show my skills") || !strcmp(line, "what are my skills") ||
      !strcmp(line, "what skills do i have") || !strcmp(line, "my aptitudes") ||
      !strcmp(line, "show aptitudes") || !strcmp(line, "attribute points") ||
      !strcmp(line, "my stats") || !strcmp(line, "stat block") ||
      !strcmp(line, "power profile") || !strcmp(line, "combat readiness") ||
      !strcmp(line, "build summary") || !strcmp(line, "risk profile") ||
      !strcmp(line, "character archetype")) {
    rewrite_command(line, "aptitudes", "");
    return;
  }
  if (!strcmp(line, "my reputation") || !strcmp(line, "show reputation") ||
      !strcmp(line, "my standing") || !strcmp(line, "show standing") ||
      !strcmp(line, "what is my reputation") || !strcmp(line, "my rep") ||
      !strcmp(line, "show rep") || !strcmp(line, "patron standing") ||
      !strcmp(line, "merchant standing")) {
    rewrite_command(line, "reputation", "");
    return;
  }
  if (!strcmp(line, "my gear") || !strcmp(line, "show gear") ||
      !strcmp(line, "my loadout") || !strcmp(line, "show loadout") ||
      !strcmp(line, "what am i wearing") ||
      !strcmp(line, "show equipment") || !strcmp(line, "my equipment")) {
    rewrite_command(line, "loadout", "");
    return;
  }
  if (!strcmp(line, "my traits") || !strcmp(line, "show traits") ||
      !strcmp(line, "my personality")) {
    rewrite_command(line, "traits", "");
    return;
  }
  if (!strcmp(line, "my momentum") || !strcmp(line, "show momentum") ||
      !strcmp(line, "how am i doing") || !strcmp(line, "am i making progress") ||
      !strcmp(line, "story so far") || !strcmp(line, "my arc") ||
      !strcmp(line, "how far have i come")) {
    rewrite_command(line, "momentum", "");
    return;
  }
  if (!strcmp(line, "my perks") || !strcmp(line, "show perks") ||
      !strcmp(line, "list perks") || !strcmp(line, "what perks do i have")) {
    rewrite_command(line, "perks", "");
    return;
  }
  if (!strcmp(line, "my voice") || !strcmp(line, "show voice") ||
      !strcmp(line, "how do i sound") || !strcmp(line, "my pronouns") ||
      !strcmp(line, "what are my pronouns") || !strcmp(line, "voice style")) {
    rewrite_command(line, "voice", "");
    return;
  }
  if (!strcmp(line, "my bio") || !strcmp(line, "show bio") ||
      !strcmp(line, "my backstory") || !strcmp(line, "tell me my story") ||
      !strcmp(line, "my biography")) {
    rewrite_command(line, "bio", "");
    return;
  }
  if (!strcmp(line, "my corruption") || !strcmp(line, "show corruption") ||
      !strcmp(line, "am i tainted") || !strcmp(line, "how corrupted am i") ||
      !strcmp(line, "moral vector") || !strcmp(line, "alignment drift")) {
    rewrite_command(line, "tainting", "");
    return;
  }
  if (!strcmp(line, "social stance")) {
    rewrite_command(line, "rapport", "");
    return;
  }
  if (!strcmp(line, "who matters here")) {
    rewrite_command(line, "rapport", "");
    return;
  }
  if (!strcmp(line, "who can i talk to") || !strcmp(line, "npc list") ||
      !strcmp(line, "list npcs") || !strcmp(line, "people in the world")) {
    rewrite_command(line, "rapport", "");
    return;
  }
  if (!strcmp(line, "logbook") || !strcmp(line, "questlog") ||
      !strcmp(line, "quest log") || !strcmp(line, "diary") ||
      !strcmp(line, "adventure log") || !strcmp(line, "adventure journal") ||
      !strcmp(line, "my journal") || !strcmp(line, "story journal")) {
    rewrite_command(line, "journal", "");
    return;
  }
  if (!strcmp(line, "my notebook") || !strcmp(line, "open my notes") ||
      !strcmp(line, "note list") || !strcmp(line, "read my notes")) {
    rewrite_command(line, "notes", "");
    return;
  }
  if (!strcmp(line, "exploration") || !strcmp(line, "exploration summary") ||
      !strcmp(line, "world progress") || !strcmp(line, "sitrep") ||
      !strcmp(line, "situation report") || !strcmp(line, "how far am i") ||
      !strcmp(line, "story progress") ||
      !strcmp(line, "where am i in the story")) {
    rewrite_command(line, "progress", "");
    return;
  }
  if (!strcmp(line, "what just happened") || !strcmp(line, "recent messages") ||
      !strcmp(line, "command recap")) {
    rewrite_command(line, "recap", "");
    return;
  }
  if (!strcmp(line, "active quests") || !strcmp(line, "current objectives") ||
      !strcmp(line, "what are my objectives") || !strcmp(line, "story goals") ||
      !strcmp(line, "checklist") || !strcmp(line, "my checklist")) {
    rewrite_command(line, "objectives", "");
    return;
  }
  if (!strcmp(line, "my hp") || !strcmp(line, "how hurt am i") ||
      !strcmp(line, "am i hurt") || !strcmp(line, "my health")) {
    rewrite_command(line, "vitals", "");
    return;
  }
  if (!strcmp(line, "mod order") || !strcmp(line, "mods order")) {
    rewrite_command(line, "mods list", "");
    return;
  }
  if (!strcmp(line, "shopping list") || !strcmp(line, "merchant stock") ||
      !strcmp(line, "shop stock") || !strcmp(line, "whats for sale") ||
      !strcmp(line, "what is for sale") || !strcmp(line, "market") ||
      !strcmp(line, "open the shop") ||
      !strcmp(line, "what is for sale here")) {
    rewrite_command(line, "wares", "");
    return;
  }
  if (!strcmp(line, "save games") || !strcmp(line, "my saves") ||
      !strcmp(line, "load slots")) {
    rewrite_command(line, "saves", "");
    return;
  }
  if (!strcmp(line, "troubleshoot") || !strcmp(line, "engine diagnostics")) {
    rewrite_command(line, "errors", "");
    return;
  }
  if (!strcmp(line, "session log") || !strcmp(line, "what went wrong") ||
      !strcmp(line, "debug log")) {
    rewrite_command(line, "errors", "");
    return;
  }
  if (!strcmp(line, "travel network") || !strcmp(line, "teleport network") ||
      !strcmp(line, "nexus list") || !strcmp(line, "attuned points")) {
    rewrite_command(line, "waypoints", "");
    return;
  }
  if (!strcmp(line, "breadcrumbs") || !strcmp(line, "my path back") ||
      !strcmp(line, "movement trail")) {
    rewrite_command(line, "trail", "");
    return;
  }
  if (!strcmp(line, "adjacent rooms") || !strcmp(line, "local map") ||
      !strcmp(line, "around here")) {
    rewrite_command(line, "nearby", "");
    return;
  }
  if (!strcmp(line, "lock status") || !strcmp(line, "door locks")) {
    rewrite_command(line, "lockcheck", "");
    return;
  }
  if (!strcmp(line, "detection risk") || !strcmp(line, "how stealthy")) {
    rewrite_command(line, "noise", "");
    return;
  }
  if (!strcmp(line, "who is here") || !strcmp(line, "anyone here")) {
    rewrite_command(line, "who", "");
    return;
  }
  if (!strcmp(line, "npc role") || !strcmp(line, "who is this npc")) {
    rewrite_command(line, "who", "");
    return;
  }
  if (!strcmp(line, "last npc role") || !strcmp(line, "who is the last npc")) {
    rewrite_command(line, "who", "");
    return;
  }
  if (!strcmp(line, "last npc attitude") || !strcmp(line, "npc attitude")) {
    rewrite_command(line, "who", "");
    return;
  }
  if (!strcmp(line, "npc danger") || !strcmp(line, "last npc danger") ||
      !strcmp(line, "how dangerous is this npc")) {
    rewrite_command(line, "who", "");
    return;
  }
  if (!strcmp(line, "npc trust") || !strcmp(line, "last npc trust")) {
    rewrite_command(line, "who", "");
    return;
  }
  if (!strcmp(line, "npc leverage") || !strcmp(line, "last npc leverage") ||
      !strcmp(line, "how much leverage do i have")) {
    rewrite_command(line, "who", "");
    return;
  }
  if (!strcmp(line, "conversation topic") ||
      !strcmp(line, "what did we talk about")) {
    rewrite_command(line, "topic", "");
    return;
  }
  if (!strcmp(line, "topic mood") || !strcmp(line, "conversation mood")) {
    rewrite_command(line, "topic", "");
    return;
  }
  if (!strcmp(line, "topic heat") || !strcmp(line, "conversation heat")) {
    rewrite_command(line, "topic", "");
    return;
  }
  if (!strcmp(line, "is last npc here") || !strcmp(line, "last npc here") ||
      !strcmp(line, "who did i last meet") || !strcmp(line, "last person i met")) {
    rewrite_command(line, "who", "");
    return;
  }
  if (!strcmp(line, "salvage") || !strcmp(line, "valuables here")) {
    rewrite_command(line, "loot", "");
    return;
  }
  if (!strcmp(line, "show inventory") || !strcmp(line, "show my inventory") ||
      !strcmp(line, "show me inventory") ||
      !strcmp(line, "show me my inventory") ||
      !strcmp(line, "what am i carrying") ||
      !strcmp(line, "what do i have")) {
    rewrite_command(line, "inventory", "");
    return;
  }
  if (!strcmp(line, "belongings") || !strcmp(line, "my stuff") ||
      !strcmp(line, "what i have on me")) {
    rewrite_command(line, "inventory", "");
    return;
  }
  if (!strcmp(line, "read room") || !strcmp(line, "read the room") ||
      !strcmp(line, "describe here")) {
    rewrite_command(line, "describe", "");
    return;
  }
  if (!strcmp(line, "surroundings") || !strcmp(line, "full room") ||
      !strcmp(line, "room description")) {
    rewrite_command(line, "describe", "");
    return;
  }
  if (!strcmp(line, "what do i see") || !strcmp(line, "what can i see") ||
      !strcmp(line, "scan room") || !strcmp(line, "scan the room") ||
      !strcmp(line, "area scan") || !strcmp(line, "survey area") ||
      !strcmp(line, "survey room") || !strcmp(line, "recon") ||
      !strcmp(line, "recon pass") || !strcmp(line, "scout") ||
      !strcmp(line, "scout area") || !strcmp(line, "room overview") ||
      !strcmp(line, "tactical readout")) {
    rewrite_command(line, "scan", "");
    return;
  }
  if (!strcmp(line, "show exits") || !strcmp(line, "list exits") ||
      !strcmp(line, "where can i go") || !strcmp(line, "where to") ||
      !strcmp(line, "connections") || !strcmp(line, "room connections")) {
    rewrite_command(line, "exits", "");
    return;
  }
  if (!strcmp(line, "where am i") || !strcmp(line, "where am i now")) {
    rewrite_command(line, "where", "");
    return;
  }
  if (!strcmp(line, "show status") || !strcmp(line, "my status")) {
    rewrite_command(line, "status", "");
    return;
  }
  if (!strcmp(line, "quick status") || !strcmp(line, "full status") ||
      !strcmp(line, "character status") || !strcmp(line, "threat posture")) {
    rewrite_command(line, "status", "");
    return;
  }
  if (!strcmp(line, "show score") || !strcmp(line, "my score") ||
      !strcmp(line, "my points") || !strcmp(line, "game stats")) {
    rewrite_command(line, "score", "");
    return;
  }
  if (!strcmp(line, "what time is it") || !strcmp(line, "what's the time") ||
      !strcmp(line, "whats the time")) {
    rewrite_command(line, "time", "");
    return;
  }
  if (!strcmp(line, "what's the weather") || !strcmp(line, "whats the weather") ||
      !strcmp(line, "how's the weather") || !strcmp(line, "hows the weather") ||
      !strcmp(line, "climate")) {
    rewrite_command(line, "weather", "");
    return;
  }
  if (!strcmp(line, "scene tone") || !strcmp(line, "atmosphere")) {
    rewrite_command(line, "weather", "");
    return;
  }
  if (!strcmp(line, "light sources") || !strcmp(line, "show lights")) {
    rewrite_command(line, "lights", "");
    return;
  }
  if (!strcmp(line, "fast travel") || !strcmp(line, "fast-travel")) {
    rewrite_command(line, "fasttravel", "");
    return;
  }
  if (!strcmp(line, "travel mood")) {
    rewrite_command(line, "progress", "");
    return;
  }
  if (!strncmp(line, "fast travel to ", 15)) {
    rewrite_command(line, "fasttravel", line + 15);
    return;
  }
  if (!strncmp(line, "fast travel ", 12)) {
    rewrite_command(line, "fasttravel", line + 12);
    return;
  }
  if (!strncmp(line, "fast-travel to ", 15)) {
    rewrite_command(line, "fasttravel", line + 15);
  ...
  if (!strncmp(line, "waystone to ", 12)) {
    rewrite_command(line, "waystone", line + 12);
    return;
  }
  if (!strncmp(line, "nexus to ", 9)) {
    rewrite_command(line, "nexus", line + 9);
    return;
  }
  ...
  if (!strncmp(line, "travel to ", 10) || !strncmp(line, "move to ", 8) ||
    ...
    if (p && rewrite_to_move_or_approach(line, p + 4)) return;
  }
  ...
  if (!strncmp(line, "search for ", 11)) {
    rewrite_command(line, "find", line + 11);
    return;
  }
  if (!strncmp(line, "find me ", 8)) {
    rewrite_command(line, "find", line + 8);
    return;
  }
  ...
  if (!strncmp(line, "tell me about ", 14)) {
    rewrite_command(line, "examine", line + 14);
    return;
  }
  if (!strncmp(line, "what is ", 8) || !strncmp(line, "what are ", 9) ||
      !strncmp(line, "who is ", 7) || !strncmp(line, "who are ", 8) ||
      !strncmp(line, "what's ", 7)) {
    char *p = strchr(line, ' ');
    if (p) {
      p++;
  ...
  if (!strncmp(line, "describe ", 9) || !strncmp(line, "check ", 6) ||
      !strncmp(line, "view ", 5) || !strncmp(line, "study ", 6) ||
      !strncmp(line, "observe ", 8)) {
    char *p = strchr(line, ' ');
    rewrite_command(line, "examine", p ? p + 1 : "");
    return;
  }
  if (!strncmp(line, "look in ", 8) || !strncmp(line, "look inside ", 12) ||
      !strncmp(line, "look into ", 10) || !strncmp(line, "look through ", 13)) {
    char *p = strchr(line, ' ');
    while (p && *p == ' ') p++;
    if (p) {
      p = strchr(p, ' ');
  ...
  if (!strncmp(line, "look for ", 9)) {
    rewrite_command(line, "find", line + 9);
    return;
  }
  ...
  if (!strncmp(line, "speak with ", 11) || !strncmp(line, "speak to ", 9) ||
      !strncmp(line, "chat with ", 10) || !strncmp(line, "chat to ", 8) ||
      !strncmp(line, "converse with ", 14) ||
      !strncmp(line, "talk with ", 10)) {
    char *p = strchr(line, ' ');
    while (p && *p == ' ') p++;
    if (p) p = strchr(p, ' ');
  ...
  if (!strncmp(line, "ask about ", 10)) {
    rewrite_command(line, "talk about", line + 10);
    return;
  }
  if (!strncmp(line, "ask ", 4)) {
    ...
    if (about) {
      *about = '\0';
      strip_trailing_space(work);
      snprintf(line, INPUT_LINE_MAX, "talk to %.*s about %.*s", 180, work,
               240, about + 7);
      strip_terminal_punctuation(line);
      return;
    }
  ...
  if (!strncmp(line, "unlock ", 7) || !strncmp(line, "open ", 5)) {
    char work[INPUT_LINE_MAX];
    char *target;
    const char *verb = !strncmp(line, "unlock ", 7) ? "unlock" : "open";
    target = line + strlen(verb) + 1;
  ...
  if (!strncmp(line, "use ", 4) || !strncmp(line, "apply ", 6)) {
    char work[INPUT_LINE_MAX];
    char *tool = line + (!strncmp(line, "use ", 4) ? 4 : 6);
  ...
  if (!strncmp(line, "pick ", 5) &&
      (strstr(line, "lock") || strstr(line, "shed"))) {
  ...
  if (!strncmp(line, "take ", 5) || !strncmp(line, "drop ", 5)) {
    char work[INPUT_LINE_MAX];
    char *item = line + 5;
    const char *verb = !strncmp(line, "take ", 5) ? "take" : "drop";
  ...
  if (!strncmp(line, "go north", 8) || !strncmp(line, "go south", 8) ||
      !strncmp(line, "go east", 7) || !strncmp(line, "go west", 7) ||
      !strncmp(line, "go up", 5) || !strncmp(line, "go down", 7)) {
    return;
  }
}
...
static void eq_bootstrap_from_character(void) {
  ...
  pc_capture(&p);
  if (p.weapon[0] && !str_ieq(p.weapon, "none")) {
    query_norm_underscore(norm, sizeof norm, p.weapon);
    ix = inv_find(norm);
    if (ix >= 0) {
      char taken[MAX_ITEM_LEN];
      (void)inv_take_out(ix, taken, sizeof taken);
      snprintf(g_eq_slots[EQ_WEAPON], sizeof g_eq_slots[EQ_WEAPON], "%s", taken);
    }
  }
  if (p.armor[0] && !str_ieq(p.armor, "none")) {
    query_norm_underscore(norm, sizeof norm, p.armor);
    ix = inv_find(norm);
    if (ix >= 0) {
      char taken[MAX_ITEM_LEN];
      (void)inv_take_out(ix, taken, sizeof taken);
      snprintf(g_eq_slots[EQ_CHEST], sizeof g_eq_slots[EQ_CHEST], "%s", taken);
    }
  }
  eq_sync_ready_item();
}
...
static void run_material_forge(const char *pending_acc, int *did_fullscreen) {
  ...
  for (;;) {
    ...
    char right_rows[15][64];
    char left_row[64], mid_row[64];
    ...
    printf("%s%s%s\n", C_BORDER, UI_RULE, C_RESET);
    printf("  %sMATERIAL FORGE%s  %s[ PROFICIENCY: %d ]%s\n", C_TITLE, C_RESET,
           C_STATUS, g_craft_proficiency, C_RESET);
    printf("%s%s%s\n", C_BORDER, UI_RULE, C_RESET);
    printf("  [ INVENTORY ]                              [ WORKBENCH: %d/6 ]                      [ PREDICTED OUTCOME ]\n",
           bench_n);
    printf("  +----------------------------------------+ +----------------------------------------+ +-----------------------------+\n");
    rows = 15;
    snprintf(right_rows[0], sizeof right_rows[0], "RESULT: %s", out_name);
    snprintf(right_rows[1], sizeof right_rows[1], "BASE STATS:");
    snprintf(right_rows[2], sizeof right_rows[2], "Durability:[%s] %2d", bd, st_dur);
    snprintf(right_rows[3], sizeof right_rows[3], "Sharpness: [%s] %2d", bs, st_shp);
    snprintf(right_rows[4], sizeof right_rows[4], "Handling:  [%s] %2d", bh, st_hnd);
    snprintf(right_rows[5], sizeof right_rows[5], "Weight:    [%s] %2d", bw, st_wgt);
    snprintf(right_rows[6], sizeof right_rows[6], "COMBINED ATTRIBUTES:");
    snprintf(right_rows[7], sizeof right_rows[7], "Hrd:%02d Shp:%02d Flx:%02d Dur:%02d", a.hrd, a.shp, a.flx, a.dur);
    snprintf(right_rows[8], sizeof right_rows[8], "Wgt:%02d Grp:%02d Bnd:%02d Utl:%02d", a.wgt, a.grp, a.bnd, a.utl);
    snprintf(right_rows[9], sizeof right_rows[9], "ANALYSIS:");
    snprintf(right_rows[10], sizeof right_rows[10], "%s", d1);
    snprintf(right_rows[11], sizeof right_rows[11], "%s", d2);
    snprintf(right_rows[12], sizeof right_rows[12], "%s", d3);
    snprintf(right_rows[13], sizeof right_rows[13], "QUALITY:%3d %s", q_score,
             q_score >= 75 ? "(fine)" : (q_score >= 50 ? "(solid)" : "(rough)"));
    snprintf(right_rows[14], sizeof right_rows[14], "PROF:%-2d READY:add/rem/craft", g_craft_proficiency);
    ...
    for (i = 0; i < rows; i++) {
      ...
      if (i < 6) {
        if (i < bench_n)
          snprintf(mid_row, sizeof mid_row, "%d. %s", i + 1, bench[i]);
        else
          snprintf(mid_row, sizeof mid_row, "%d. [ Empty ]", i + 1);
      }
      ui_print_tripanel_row(left_row, mid_row, right_rows[i]);
    }
    printf("  +----------------------------------------+ +----------------------------------------+ +-----------------------------+\n");
    printf("  Commands: [ADD #] [REM #] [PROF #] [CRAFT] [CLEAR] [DONE]\n");
    ...
    if (!line[0]) continue;
    if (!strcmp(line, "done") || !strcmp(line, "back") || !strcmp(line, "exit") ||
        !strcmp(line, "resume")) {
      break;
      ...
      for (i = 0; i < bench_n; i++) (void)inv_remove_exact_one(bench[i]);
      inv_add(made);
...
static void run_equipment_inventory_ui(const char *pending_acc, int *did_fullscreen) {
  enum { EQ_LEFT_W = 50, EQ_RIGHT_W = 66, EQ_ROWS = 16 };
  char line[INPUT_LINE_MAX];
  char msg[200];
  for (;;) {
    ...
    for (i = 0; i < EQ_SLOT_COUNT; i++) {
      ...
      craft_profile_for_item(g_eq_slots[i], &p);
      def += (p.hrd / 3) + (p.dur / 4);
      if (i == EQ_WEAPON) atk += (p.shp / 2) + (p.hrd / 4) + 1;
      else if (i == EQ_OFFHAND) atk += p.shp / 6;
      else atk += p.shp / 10;
      wgt += p.wgt / 2;
    }
    for (i = 0; i < EQ_ROWS; i++) {
    ...
    for (i = 0; i < EQ_SLOT_COUNT; i++) {
      ...
      if (g_eq_slots[i][0]) {
        CraftMatProfile sp;
        char stat[24];
        craft_profile_for_item(g_eq_slots[i], &sp);
        sdef = (sp.hrd / 3) + (sp.dur / 4);
        if (i == EQ_WEAPON) satk = (sp.shp / 2) + (sp.hrd / 4) + 1;
        else if (i == EQ_OFFHAND) satk = sp.shp / 6;
        else satk = sp.shp / 10;
        ui_line_append_width(&lb, g_eq_slots[i], 20);
    ...
    snprintf(left_rows[12], sizeof left_rows[12], "+------------------------------------------------+");
    {
      ...
      ui_line_pad(&lb);
      snprintf(msg, sizeof msg, "|%s|", left_rows[13]);
      snprintf(left_rows[13], sizeof left_rows[13], "%s", msg);
    }
    {
      UiLineBuf lb;
      char tmp[80];
      ...
      ui_line_append(&lb, tmp);
      ui_line_pad(&lb);
      snprintf(msg, sizeof msg, "|%s|", left_rows[14]);
      snprintf(left_rows[14], sizeof left_rows[14], "%s", msg);
    }
    ...
    snprintf(right_rows[0], sizeof right_rows[0], "+----------------------------------------------------------------+");
    {
      ...
      ui_line_pad(&rb);
      snprintf(msg, sizeof msg, "|%s|", right_rows[1]);
      snprintf(right_rows[1], sizeof right_rows[1], "%s", msg);
    }
    {
      UiLineBuf rb;
      ...
      ui_line_append(&rb, "----------------------------------------------------------------");
      ui_line_pad(&rb);
      snprintf(msg, sizeof msg, "|%s|", right_rows[2]);
      snprintf(right_rows[2], sizeof right_rows[2], "%s", msg);
    }
    for (i = 0; i < 12; i++) {
      if (i < g_inv_n) {
        CraftMatProfile p;
        int idef, iatk, slot_ix;
        const char *slot_name = "misc";
        UiLineBuf rb;
        char idchunk[8], typechunk[13], atchunk[32], namechunk[96];
        craft_profile_for_item(g_inv[i], &p);
        idef = (p.hrd / 3) + (p.dur / 4);
        iatk = (p.shp / 2) + (p.hrd / 5);
        slot_ix = eq_infer_slot(g_inv[i]);
        if (slot_ix >= 0 && slot_ix < EQ_SLOT_COUNT) slot_name = EQ_SLOT_NAME[slot_ix];
        ui_line_init(&rb, right_rows[i + 3], sizeof right_rows[i + 3], 64);
        snprintf(idchunk, sizeof idchunk, "[%2d] ", i + 1);
        snprintf(typechunk, sizeof typechunk, "(%s)", slot_name);
        snprintf(atchunk, sizeof atchunk, "Def:%02d Atk:%02d W:%02d", idef, iatk, p.wgt / 2);
        snprintf(namechunk, sizeof namechunk, "%s%s", idchunk, g_inv[i]);
        ui_line_append(&rb, " ");
        ui_line_append_width(&rb, namechunk, 27);
        ui_line_append_width(&rb, typechunk, 12);
        ui_line_append(&rb, " ");
        ui_line_append_width(&rb, atchunk, 23);
        ui_line_pad(&rb);
        snprintf(msg, sizeof msg, "|%s|", right_rows[i + 3]);
        snprintf(right_rows[i + 3], sizeof right_rows[i + 3], "%s", msg);
        idx_map[listed++] = i;
        shown++;
      }
    }
    for (i = 3 + shown; i < 15; i++) {
      UiLineBuf rb;
      ui_line_init(&rb, right_rows[i], sizeof right_rows[i], 64);
      ui_line_pad(&rb);
      snprintf(msg, sizeof msg, "|%s|", right_rows[i]);
      snprintf(right_rows[i], sizeof right_rows[i], "%s", msg);
    }
    snprintf(right_rows[15], sizeof right_rows[15], "+----------------------------------------------------------------+");
    ...
    for (i = 0; line[i]; i++) line[i] = (char)tolower((unsigned char)line[i]);
    if (!line[0]) continue;
    if (!strcmp(line, "done") || !strcmp(line, "back") || !strcmp(line, "exit")) break;
    ...
    if (!strncmp(line, "equip ", 6)) {
      ...
      if (n < 1 || n > listed) {
        ui_block_pause("EQUIPMENT", "Use an inventory row number from this screen.");
        continue;
      }
      if (slot < 0) slot = eq_infer_slot(g_inv[idx_map[n - 1]]);
      eq_equip_from_inventory_index(idx_map[n - 1], slot, msg, sizeof msg);
      ui_block_pause("EQUIPMENT", msg);
      continue;
    }
...
static void process_command(char *line, char *msg, size_t msgcap,
  ...
  *turn_advance = 1;
  ...
  if (!strcmp(line, "menu") || !strcmp(line, "mainmenu") ||
      !strcmp(line, "main menu")) {
    *turn_advance = 0;
    run_game_menu();
    *did_fullscreen = 1;
    return;
  ...
  if (!strcmp(line, "help modding") || !strcmp(line, "help mods")) {
    *turn_advance = 0;
    ui_modding_guide_pager(pending_acc, did_fullscreen);
    return;
  }

  if (!strcmp(line, "forge") || !strcmp(line, "crafting")) {
    *turn_advance = 0;
    run_material_forge(pending_acc, did_fullscreen);
    return;
  }

  if (!strcmp(line, "clear") || !strcmp(line, "cls")) {
    *turn_advance = 0;
    paint_normal();
    return;
  }

  if (!strcmp(line, "verbose") || !strcmp(line, "long")) {
    *turn_advance = 0;
    g_verbose_room = 1;
    snprintf(msg, msgcap, "Full room descriptions on (main window).");
    return;
  }
  if (!strcmp(line, "brief") || !strcmp(line, "short")) {
    *turn_advance = 0;
    g_verbose_room = 0;
    snprintf(msg, msgcap,
  ...
  if (!strcmp(line, "recap") || !strcmp(line, "transcript")) {
    *turn_advance = 0;
  ...
  if (!strcmp(line, "recap clear") || !strcmp(line, "clear recap")) {
    *turn_advance = 0;
    g_recap_n = 0;
  ...
  if (!strcmp(line, "lights") || !strcmp(line, "lighting")) {
    *turn_advance = 0;
  ...
  if (!strcmp(line, "about") || !strcmp(line, "credits")) {
    *turn_advance = 0;
  ...
  if (!strcmp(line, "version") || !strcmp(line, "ver")) {
    *turn_advance = 0;
    snprintf(msg, msgcap,
  ...
  if (!strcmp(line, "mods reload") || !strcmp(line, "reload mods")) {
    *turn_advance = 0;
  ...
  if (!strncmp(line, "mods directory ", 15) ||
      !strncmp(line, "mods path ", 10)) {
    const char *p = line + (!strncmp(line, "mods directory ", 15) ? 15 : 10);
    *turn_advance = 0;
    while (*p == ' ') p++;
    if (!p[0]) {
      snprintf(msg, msgcap,
               "Usage: mods directory <folder>  (full path to mod packs root)");
      return;
    }
  ...
  if (!strcmp(line, "mods list") || !strcmp(line, "list mods")) {
    *turn_advance = 0;
  ...
  if (!strcmp(line, "mods doctor") || !strcmp(line, "mods verify") ||
      !strcmp(line, "mods repair")) {
    char mp[520];
    *turn_advance = 0;
    mods_resolve_root(mp, sizeof mp);
    aet_mod_bootstrap_prepare_runtime(g_save_path, mp, &g_bootstrap_status);
  ...
  if (!strcmp(line, "inventory") || !strcmp(line, "i") ||
  ...
  if (!strcmp(line, "coins") || !strcmp(line, "wallet") ||
      !strcmp(line, "money")) {
    *turn_advance = 0;
  ...
  if (!strcmp(line, "trail clear") || !strcmp(line, "clear trail")) {
    *turn_advance = 0;
    g_hist_n = 0;
  ...
  if (!strcmp(line, "progress") || !strcmp(line, "visited") ||
      !strcmp(line, "seen")) {
    *turn_advance = 0;
  ...
  if (!strcmp(line, "why blocked") || !strcmp(line, "why cant i move") ||
      !strcmp(line, "why can't i move")) {
    *turn_advance = 0;
    format_why_blocked_body(body, sizeof body);
    ui_fullscreen("WHY BLOCKED", body, pending_acc, did_fullscreen);
    return;
  }
  if (!strcmp(line, "causality movement") || !strcmp(line, "causality social") ||
      !strcmp(line, "causality save") || !strcmp(line, "causality parser")) {
  ...
  if (!strncmp(line, "causality ", 10) || !strncmp(line, "because ", 8)) {
    const char *q = !strncmp(line, "causality ", 10) ? line + 10 : line + 8;
    *turn_advance = 0;
    while (*q == ' ') q++;
    format_causality_body(body, sizeof body, q);
    ui_fullscreen("CAUSALITY", body, pending_acc, did_fullscreen);
  ...
  if (!strcmp(line, "errors clear") || !strcmp(line, "diagnostics clear")) {
    *turn_advance = 0;
    diag_clear();
  ...
  if (!strncmp(line, "wait until ", 11) || !strncmp(line, "rest until ", 11)) {
  ...
  if (!strcmp(line, "nearby") || !strcmp(line, "map") ||
      !strncmp(line, "nearby ", 7)) {
    const char *mode = NULL;
    *turn_advance = 0;
  ...
  if (!strcmp(line, "waypoints") || !strcmp(line, "waypoint") ||
      !strcmp(line, "waystones") || !strcmp(line, "waystone") ||
      !strcmp(line, "nexus") || !strcmp(line, "fasttravel")) {
    *turn_advance = 0;
  ...
  if (!strncmp(line, "waypoint ", 9) || !strncmp(line, "waystone ", 9) ||
    ...
    const char *r = !strncmp(line, "nexus ", 6) ? line + 6
                    : !strncmp(line, "fasttravel ", 11) ? line + 11
                                                        : line + 9;
    while (*r == ' ') r++;
  ...
  if (!strcmp(line, "describe") || !strcmp(line, "blurb") ||
      !strcmp(line, "room")) {
    *turn_advance = 0;
  ...
  if (!strncmp(line, "goto ", 5) || !strncmp(line, "approach ", 9) ||
    ...
    if (!strncmp(line, "goto ", 5))
      r = line + 5;
    else if (!strncmp(line, "approach ", 9))
      r = line + 9;
    else if (!strncmp(line, "walk to ", 8))
      r = line + 8;
    else
      r = line + 6;
    while (*r == ' ') r++;
  ...
  if (!strncmp(line, "equip ", 6) || !strncmp(line, "hold ", 5) ||
    ...
    const char *r =
        !strncmp(line, "equip ", 6) ? line + 6
        : !strncmp(line, "wield ", 6) ? line + 6
                                        : line + 5;
    *turn_advance = 0;
